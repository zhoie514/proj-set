    ...
    def gen_result(self, dt: pd.DataFrame):
        ...
        for col, row in dt.iterrows():
            try:
                ...
                try:
                    print(
                        "换行:", f"{row['production_code']} -> f{dt.at[col + 1, 'production_code']}")
                except KeyError as e:
                    print()
            if row['log_type'] == "realnameauth":
                self.zx_sum += int(row['count'])
                if str(int(row['rsp_code'])) == "1" or len(
                        str(int(row['rsp_code']))) > 3:
            ...
            if row['log_type'] == "credit":
                ...
                if row['result'] == "reject" and str(
                        int(row['rsp_code'])) == "0":
                    self.sx_ilog_reject += int(row['count'])
                if row['err'] != "antifraud reject" and str(
                        int(row['rsp_code'])) == "1":
                    self.sx_request_failed += int(row['count'])
                if len(str(
                        int(row['rsp_code']))) > 2 and row['production_code'] not in CONF.CREDIT10087:
                    self.sx_request_failed += int(row['count'])
            ...
            if row['log_type'] == 'withdraw':
                ...
                if row['err'] in CONF.REQUEST_FAILED or len(
                        str(int(row['rsp_code']))) > 2:
                    self.tx_request_failed += int(row['count'])
                if str(row['production_code']
                       ) == "SQC" and row['result'] == "success":
            ...
            if row['log_type'] == 'withdraw_query':
                ...
                if row['production_code'] != "HB":  # 还呗的支付通道拒绝不从这个事件记录里面统计
                    ...
                    if row['err'] == "放款失败——ilog拒绝":
                        ...
                        self.tx_ilog_reject -= self.tx_amtques
            ...
            for grid in range(1, 1000):
                ...
                if str(
                    ...
                    for col in range(0, len(data_row)):
                        single_sheet.cell(
                            row=grid, column=col + 1).value = data_row[col]
                    logging.info(f" {dic['wb_name']} 覆盖写入")
                    break
                if not blank_row.value:
                    for col in range(0, len(data_row)):
                        single_sheet.cell(
                            row=blank_row.row, column=col + 1).value = data_row[col]
...
class FixExcel:
    ...
    def __init__(self, source_code: str):
        self.channel = CONF.PROD_CHANNEL.get(source_code, False)
    ...
    def do_fix(self):
        column_loan_succ = -999
        column_pay_fail = -999
        column_pay_rate = -999
        column_loan_sum = -999
        if self.sourcecode in ("HB"):
            column_loan_succ = 20
            column_pay_fail = 24
            column_pay_rate = 25
            column_loan_sum = 18
        elif self.sourcecode in ("cangu", "LZC"):
            column_loan_succ = 11
            column_pay_fail = 15
            column_pay_rate = 16
            column_loan_sum = 9
        ...
        for x in range(3, 1000):
            ...
            if str(self.sheet.cell(row=x, column=1).value) == self.date:
                self.sheet.cell(
                    row=x, column=column_loan_succ).value = self.loan_succ
                self.sheet.cell(
                    row=x, column=column_pay_fail).value = self.pay_fail
                try:
                    self.sheet.cell(
                        row=x, column=column_pay_rate).value = self.loan_succ / int(
                        self.sheet.cell(
                            row=x, column=column_loan_sum).value or 0)
                except ZeroDivisionError:
                    print(f"{self.sourcecode} 昨天没有放款成功或失败")
                break
...
class DownEmail:
    ...
    def __init__(
            self,
            acct=CONF.DOWN_EMAIL_ACCT,
            pwd=CONF.DOWN_EMAIL_PWD,
            host=CONF.DOWN_EMAIL_HOST):
        self._acct = acct
        self._pwd = pwd
        self._host = host
        try:
            self._server = poplib.POP3_SSL(
                self._host, port=poplib.POP3_SSL_PORT)
    ...
    def run(self):
        ...
        for i in range(
                len(mails), len(mails) - CONF.REC_NUM, -1):  # 遍历最新的N个邮件就够了，不用看太多
            ...
            try:
                if re.match(".*<(.*)>", msg.get("From")
                            ).group(1) != "drbigdata@xwfintech.com":
                    continue
            except BaseException:
                continue
            ...
            # 邮件的日期
            try:
                mail_time = time.strptime(
                    msg.get("Received")[-31:-6], '%a, %d %b %Y %H:%M:%S')
            except BaseException:
                mail_time = time.strptime(
                    msg.get("Date")[
                    0:24], '%a, %d %b %Y %H:%M:%S')
            ...
            if mail_time != today:
                # 不是当天的邮件就跳过，这样可以设置下载前几天的附件  判断邮件发送日期是否 =
                # today(依赖conf里面的dayoffset)
                continue
...
def send_email(selfy: str = "", out: str = ""):
    ...
    if selfy:
        ...
        message_selfy.attach(
            MIMEText(
                f"{commit}内部的助贷方统计结果\r\n数据源为大数据每日推送的cros_log清洗统计结果.xlsx\r\n内部邮件:所有的统计表格\r\n"
                "外部邮件:HB.xlsx,TPJF.xlsx,LX.xlsx,WX.xlsx,SQ.xlsx,QH.xlsx,LZ.xlsx,cangu.xlsx, 导流_业务数据统计_#日期#.xlsx, 助贷_业务数据统计_#日期#.xlsx)\r\n  滨农: YQG.xlsx"))
        attr_selfy = MIMEText(
            open(
                os.path.join(
                    CONF.ZIP_OUT,
        ...
        message_selfy['From'] = formataddr(
            ["Public-数据推送", "publica@xwfintech.com"])
...
def auto_run(fix=("HB", "cangu", "LZC")):
    ...
    res = tool.get_result()
    for k in res:
        # 联众以及灿谷没有单独走征信,没有realnameAuth事件,他们发起授信请求并成功后,就会去查
        # 故把联从和灿谷的前几列数据删掉
        if k.upper() == "CANGU" or k.upper() == "LZC":
            res[k][1: 10] = []
            ...
        print(k, ":", res[k])
    myexcel = MyExcel(res)
    ...
    fix_inst.do_fix()
    fix_inst = FixExcel(fix[1])
    fix_inst.do_fix()
    fix_inst = FixExcel(fix[2])
    fix_inst.do_fix()
